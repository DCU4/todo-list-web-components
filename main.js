...
class TodoItem extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this._chk.addEventListener('change', () => {
      ...
      postTodo(this.textContent, this._chk.checked).then(() => {
    ...
    this._del.addEventListener('click', () => {
      ...
      deleteTodo(this.textContent).then(() => {
...
async function postTodo(item, done = false) {
  ...
  let notes = data.notes.note ? data.notes.note.split('\n') : [];
  if (done) {
    const idx = notes.findIndex(n => n === item || n === item + ' (done)');
    if (idx !== -1) {
      notes[idx] = item + ' (done)';
    }
  }
...
async function deleteTodo(item) {
  const data = await fetchTodos();
  let notes = data.notes.note ? data.notes.note.split('\n') : [];
  notes = notes.filter(n => n !== item && n !== item + ' (done)');
  const updatedNotes = notes.join('\n');
  const response = await fetch('https://notes-api-five.vercel.app/685a0963e9b0630007bd0c84/?_method=PUT', {
      method: "POST",
      body: "note="+updatedNotes,
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
      }
  });
